...
mod tests {
    ...
    fn reverse_queue_order() {
        ...
        assert_eq!(execution_order(tasks.clone()), vec![44, 43, 42]);
        assert_eq!(execution_order_original(tasks), vec![44, 43, 42]);
    }
    ...
    fn two_items_queued_at_once() {
        ...
        assert_eq!(execution_order(tasks.clone()), vec![42, 44, 43]);
        assert_eq!(execution_order_original(tasks), vec![42, 44, 43]);
    }
    ...
    fn idle() {
        ...
        assert_eq!(execution_order(tasks.clone()), vec![42, 43]);
        assert_eq!(execution_order_original(tasks), vec![42, 43]);
    }

    #[test]
    fn empty_task_list() {
        assert_eq!(execution_order(vec![]), vec![]);
        assert_eq!(execution_order_original(vec![]), vec![]);
    }
    ...
    fn task_inserted_into_queue() {
        ...
        assert_eq!(execution_order(tasks.clone()), vec![42, 43, 45, 44]);
        assert_eq!(execution_order_original(tasks), vec![42, 43, 45, 44]);
    }
}
